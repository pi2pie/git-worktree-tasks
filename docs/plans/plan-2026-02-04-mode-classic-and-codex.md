...
- [x] Codex-mode worktree discovery primitives:
  ...
  - [x] Derive `<opaque-id>` as **the first path segment** under `$CODEX_HOME/worktrees` (e.g., `$CODEX_HOME/worktrees/bf15/<repo>` → `bf15`).
- [x] Implement codex-mode behavior for read-only commands:
  ...
  - [x] In classic mode, hide codex worktrees by default (treat detached HEAD under `$CODEX_HOME/worktrees` as codex-owned).
